...
const ReusablePriorityPage = ({ priority }: Props) => {
  ...
  const userId = 1; // replace with user id when implemented in backend
  ...
  if(isTasksError || !tasks) return console.error(isTasksError);
  ...
  return (
    <div className='m-5 p-4'>
        ...
        <div className='mb-4 flex justify-start'>
            <button
                className={`px-4 py-2 ${
                    view === 'list' ? 'bg-gray-300 font-bold' : 'bg-white'
                } rounded-l`}
            ...
            <button
                className={`px-4 py-2 ${
                    view === 'table' ? 'bg-gray-300 font-bold' : 'bg-white'
                } rounded-r`}
